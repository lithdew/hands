...
Observed gate time in the initial pass had a **364.5 ms median and 300–907 ms range** across the 12 requests. Pi's six requests had a 369.5 ms median (328–906 ms); standalone's six had a 334.5 ms median (300–907 ms). The initial three requests took 868–907 ms and remain included. An earlier restricted-runner attempt produced 12 transport failures and zero model judgments; those failures are excluded from policy accuracy and timing statistics. These authored development fixtures are not a safety benchmark, latency distribution or evidence of complete Gmail success. The live task's eventual send still needs its own observed confirmation.

Two additional sequential passes of the tracked command returned **24/24 valid, expected decisions**, with no unnecessary approval pauses or false allows. Together the three passes matched expectations on 36/36 requests, still only six distinct fixtures. Pi's delegated-wording authorization scores were **0.90, 0.91, 0.90**; standalone returned **0.93** each time. The Pi result therefore remains exactly on the threshold in two runs, despite no observed false pause. Thresholds remain unchanged. The additional passes had medians of 350.5 ms (307–971 ms) and 377 ms (330–858 ms), including each run's initial requests. These repeats check local consistency, not broader accuracy or a dependable margin above the approval threshold.

The [tracked synthetic record](authorization-smoke-2026-09-19.json) preserves cases, all scores and timings. Local raw results and the transport-failure record are under ignored `out/evals/authorization-smoke/`. The tracked [authorization eval](../jev/authorization.eval.ts) prints a dry run with `bun jev/authorization.eval.ts`. To repeat the same 12 paid Jev calls, use `bun jev/authorization.eval.ts --live --out=out/evals/authorization-recheck` with a fresh run directory. It permits at most three concurrent calls, disables retries, performs no UI action and records invalid trials separately from successful policy judgments. Deterministic tests additionally cover full body tails, raw permission changes during gates, incomplete speech, malformed contracts, changed recipients, and batch field updates with an unchanged structural fingerprint.
