...
| starts an app | `ShellExecuteEx` with `SW_SHOWNOACTIVATE`, by executable, or by name through its Start Menu shortcut or its packaged AppID (`shell:AppsFolder\…`, as the Start menu starts Claude or Spotify); the window is found as it appears, since the pid that comes back can be a stub (Notepad, Calculator) | nothing |
...
Each hand works on a virtual desktop of its own ("Hands: Lefty", made as it opens its first window and removed as it ends; switch to it to watch), and the app it starts is moved there as its window appears and probed once (still there, a picture with something in it, a tree that kept at least half its labels). A window the desktop does not work for is brought back and sunk behind your windows, and that app stays there for the rest of the run, with one note to the hand as the app opens: a UWP app such as Calculator, which Windows freezes on a desktop that is not shown, is known and skips the probe; a window the shell will not take, or brings back a fourth time (by its own account of where the window is, so that your switching to the hand's desktop to watch is not a bringing back), is given up on the same way, quietly. Its browser window joins that desktop once your Chrome runs with `--disable-features=CalculateNativeWinOcclusion` (Chrome treats a window on another desktop as covered, and stops painting it and building its page tree; the flag turns that tracking off), and otherwise stays on the current desktop behind yours, as before. An app you already have open is started again for a window of the hand's own, which most apps open in the running instance, so the hand never works in your window (one did, once, in a Notepad with `.env` open); only an app that opens no second window is taken up as it is. A window of the hand's that lies behind yours is put back there after each action when it has climbed (a Chrome window does after its first read, Notepad on a press). `HANDS_DESKTOP=0` turns the desktops off.
