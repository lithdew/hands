...
export type Ask = <const Q extends Questions>(state: EntryType, questions: Q) => Promise<Answers<Q>>;
...
export function assertContract<Q extends Questions>(questions: Q, answers: unknown): asserts answers is Answers<Q> {
  ...
  for (const [name, question] of Object.entries(questions)) {
    ...
    if (!answer || answer.type !== question.type) {
      throw new ContractError(`"${name}": expected a ${question.type} answer, got ${JSON.stringify(answer)}`);
    }
    if (question.type === "noul") {
      if (!isProbability(answer.noul)) throw new ContractError(`"${name}": noul ${answer.noul} is not within 0..1`);
    } else if (question.type === "choice") {
      if (typeof answer.choice !== "string" || !Object.hasOwn(question.criteria, answer.choice)) {
        throw new ContractError(`"${name}": "${answer.choice}" is not one of the offered labels`);
      }
      if (!isProbability(answer.confidence)) throw new ContractError(`"${name}": confidence is not within 0..1`);
    } else if (typeof answer.score !== "number" || Number.isNaN(answer.score)) {
...
async function main(argv: string[]) {
  ...
  switch (argv[0]) {
    case "models": {
      for (const m of await new TypeSafeClient().models.list()) console.log(`${m.name}\t${m.description}`);
      return;
    }
    case "ping": {
