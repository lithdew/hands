...
export async function sift<T extends { text: string }>(ask: Ask, goal: string, items: T[], what: string, opts: { keep?: number; atLeast?: number } = {}): Promise<Sifted<T>[]> {
  ...
  return (passed.length ? passed : scored.slice(0, 3)).slice(0, opts.keep ?? scored.length);
}
