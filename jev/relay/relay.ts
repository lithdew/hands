...
export type Trace = { jevRequests: number; jevMs: number; llmCalls: { what: string; ms: number }[]; fetched: number; sifted: number; kept: number; redone: string[]; ms: number };
...
Never plan anything that publishes, sends, buys or signs in.` })) as Plan;
  ...
  async function write(step: Step, feedback: string[]): Promise<string> {
    ...
    const out = (await llm(`write:${step.id}`, { model: deps.deepModel ?? DEEP, effort: "medium", schema: FILES_SCHEMA, user: JSON.stringify({ request: task, goal: step.goal, must_be_true: step.accept, not_yet_true_last_time: feedback, notes, files_so_far: Object.keys(ws.files) }), system:
      `You write the files for one step of a task. ${deps.kit.brief}
Use only facts that are in the notes, with their addresses where the format has a place for sources. Where the notes lack something, say so in the file instead of inventing it. Paths are relative, inside the workspace. Return every file in full.` })) as { files: { path: string; content: string }[] };
