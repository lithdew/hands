...
if (first === "judge") { await rm(join(dirOf(second!), "judgement.json"), { force: true }); show(await judge(second!, dirOf(second!), llm)); process.exit(0); }
...
const result = await relay(spec.said, ws, { ask: createJev(), llm: process.argv.includes("--cache-llm") ? remembered(llm) : llm, kit: KITS[spec.kit]! });
