...
// The writer works from the notes alone (`prepare` gives it nothing more): the review holds every sentence of the page to the notes.
export const site: Kit = { name: "site", brief: BRIEF, context: ownerContext, sources: () => ownSources(), requery: true, admit: (ctx, step, passages) => admit(ctx.ask, passages, step, ctx.ws), prepare: async () => ({}), review: (ctx) => review(ctx.ask, ctx.ws), build };
