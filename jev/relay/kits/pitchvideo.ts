...
PLANNING: research here reads THIS REPOSITORY'S OWN DOCUMENTS (its READMEs and evaluation reports), never the web (give each research step one query, its subject in a few words; nothing is searched with it). The kit supplies the documents as some four hundred passages, each with an address such as docs/report.md:57, and a fast reader scores every passage against the step's goal, so a goal names ONE kind of passage, narrowly. Plan four research steps that do not need each other: (1) what the product is, and the problem it answers: what goes wrong, or is slow, for a user of such tools without it; (2) how it works: what KIND of model each model is (what it can and cannot output), which kind of decision each takes, how many of them and how fast, which few things are left to the slower, more general models, and where the work runs while the user keeps working; (3) its strongest measured results from its evaluations: whole tasks solved and with how few model calls and how little time, accuracy before and after a design change, time per request, each with what was measured, on what, and how many cases; (4) what it has demonstrably done live on real applications, and how that was confirmed. Then exactly ONE write step that needs all four and writes video/script.json, then ONE build step.
THE SCRIPT is a pitch as for a hackathon jury, spoken by the team about its own project: a hook; the problem (what is wrong or slow today without the product, as pains, not the product's benefits); what the product is; how it works, so that a jury comes away knowing the division of labour: what kind of model each one is, which takes the many small decisions and how fast one is, which few big things are left to the slower, more general models, and where the work happens while the user keeps working; then the proof, one task shown being done, and a close with an ask. Choose the proof by strength: whole tasks finished, with how few model calls and how little time; a before-and-after gain from a design decision; live runs on real applications. One figure to a scene, never generic praise, and not a scene of limitations. ${LIMITS.scenes[0]} to ${LIMITS.scenes[1]} scenes, ${LIMITS.words[0]} to ${LIMITS.words[1]} words of narration in all (the video runs at ${WPM} words a minute, so that is a minute and a half to two minutes). The narration is shown as captions: write it to be read aloud, in plain confident sentences ("we", the product's name, active verbs). Be honest through precision, not through hedging: say "on simulated apps" or "on 12 held-out synthetic screens" where that is the scope, and never talk about "the documents", "the repository" or "the evidence" in the narration.
