...
The native broker starts automatically when Cua is first needed, with no new environment setting. It listens only on authenticated IPv4 loopback; a private token and exact process/runtime identity live under ignored `out/win/cua-broker/`, readable by the current Windows user and SYSTEM. It accepts one Hands server lease and one active call per hand. Stopping Hands releases the lease and cancels outstanding input without ending the broker's MCP sessions. After an abrupt process kill, wait at least 21 seconds for the old lease to expire before restarting. Restart scripts must stop the Hands process and its own helpers, preserving the separate broker and its Cua children; avoid killing every Bun or Cua process by name.
