...
## Signed-in Chrome uses the direct backend

**My Chrome** now uses `chrome-browser.ts`, `chrome-cdp.ts` and `chrome-observation.ts`. It does not call Cua for attachment, observations, input, dialogs or browser previews. Cua remains lazy native-app support. `/status` exposes `backend: "chrome-cdp"` for this selection.
...
Cua's installer may register a `cua-driver-serve` autostart task, which Hands does not use. Native-app calls use the existing authenticated loopback broker and its per-hand MCP connections. Use `bun win/cua-broker.ts status` to inspect that broker; stop Hands before `bun win/cua-broker.ts stop`. Preserve unrelated browser and helper processes when restarting. An abrupt kill may leave the old broker lease alive for 21 seconds; the direct Chrome backend does not acquire that lease.
