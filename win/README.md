...
The installer registers a `cua-driver-serve` autostart task. Puk does not use it. On native Windows, a hidden Puk broker owns one private `cua-driver mcp` transport per hand. The broker survives a Hands server restart, retaining each hand's public browser session and Cua's in-memory grant. WSL currently retains the direct per-server MCP transport. Remove the unrelated installer task from an elevated PowerShell with `cua-driver autostart disable`.

The native broker starts automatically when Cua is first needed, with no new environment setting. It listens only on authenticated IPv4 loopback; a private token and exact process/runtime identity live under ignored `out/win/cua-broker/`, readable by the current Windows user and SYSTEM. It accepts one Hands server lease and one active call per hand. Stopping idle Hands releases the lease without ending the broker's MCP sessions. If cancellation or an SDK timeout interrupts dispatched input, the affected hand is quarantined until its owned Cua process exits; reconnecting that hand explicitly can then require Chrome approval again. A cancelled client promise alone cannot prove that input stopped. After an abrupt process kill, wait at least 21 seconds for the old lease to expire before restarting. Restart scripts must stop the Hands process and its own helpers, preserving the separate broker and its Cua children; avoid killing every Bun or Cua process by name.

Use `bun win/cua-broker.ts status` to inspect the broker without browser input. Stop Hands, then run `bun win/cua-broker.ts stop` to explicitly end all broker sessions. Choosing the private browser instead of **My Chrome** ends that hand's existing-browser session. A changed broker source/runtime or an unresponsive live broker fails closed; stop the old broker explicitly before upgrading it. An incomplete startup retains `launch.lock` rather than spawning a competing process. Inspect the recorded launcher/child before removing a stale lock.

Your instruction authorizes Puk to use the selected Chrome window; Chrome may still require its own remote-debugging approval. Keeping Cua's transport and public session avoids losing its approval state merely because Hands restarts. Fewer Chrome prompts still needs verification against the installed driver/browser. Grants can expire (the inspected Cua source uses a 30-minute idle limit and an eight-hour absolute limit), and browser or broker restarts can require approval again. The broker never prepares a browser by itself, extends grants with background browser polling, or retries an input whose response was lost. An expired restoration stays disconnected until an explicit attachment.
