...
Jev still does not drive native controls directly. Earlier background UI Automation snapshots exposed almost nothing in Calculator and only a title bar in Paint, taking 1 to 3 s per look. The new Pi adapter tries Cua's native element tokens and snapshot references, with screenshots available when labels are insufficient; this does not establish that every background Windows app is controllable. `PUK_JEV_FIRST=0` sends every task directly to Pi while keeping the compact tools available.
