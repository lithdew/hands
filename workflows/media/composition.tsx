...
function EvidenceContent({scene, frame}: {scene:PreparedScene;frame:number}) {
  ...
  const source=evidence.length ? evidence[Math.min(evidence.length-1,Math.floor(frame/(scene.frames*.62)*evidence.length))]!.source : scene.artifactLabel;
  return <div data-evidence-layout={phase} style={{position:"absolute",left:58,width:1164,top:62}}>
    ...
    {phase==="artifact" ? <>
      ...
      <div data-evidence-image-panel style={{height:448,width:"100%",background:"#152334",border:"1px solid #284254",borderRadius:9,overflow:"hidden"}}><Img src={staticFile(scene.imageAsset!)} style={{width:"100%",height:"100%",objectFit:"contain"}}/></div>
      {source&&<div style={{marginTop:8,fontSize:18,lineHeight:1.2,color:colors.muted,overflowWrap:"anywhere"}}><EvidenceText height={44} identity={`${scene.id}-${source}`}>{source}</EvidenceText></div>}
    </> : <div style={{marginTop:26}}><EvidenceText height={450} identity={`${scene.id}-details`}>
