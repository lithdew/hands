...
function MatrixText({children, sceneId}: {children: React.ReactNode; sceneId: string}) {
  ...
  return <div data-matrix-text-panel style={{position:"absolute",top:25,left:0,width:435,height:MATRIX_TEXT_HEIGHT}}>
...
function EvidenceContent({scene, frame}: {scene:PreparedScene;frame:number}) {
  ...
  return <div data-evidence-layout={phase} style={{position:"absolute",left:58,width:1164,top:52}}>
    ...
    {phase==="artifact" ? <>
      ...
      <div data-evidence-image-panel style={{height:396,width:"100%",background:"#152334",border:"1px solid #284254",borderRadius:9,overflow:"hidden"}}><Img src={staticFile(scene.imageAsset!)} style={{width:"100%",height:"100%",objectFit:"contain"}}/></div>
      ...
      {scene.body&&<p style={{fontSize:30,lineHeight:1.3,color:colors.muted,margin:"0 0 22px"}}>{scene.body}</p>}
...
function Scene({scene, index, total, editorial=false}: {scene: PreparedScene;index:number;total:number;editorial?:boolean}) {
  ...
  const heading=<h1 style={{fontSize:scene.visual==="title"||scene.visual==="closing"?64:matrixMode?43:46,lineHeight:1.1,fontWeight:650,maxWidth:matrixMode?435:1130,margin:matrixMode?"12px 0 16px":"16px 0 20px",letterSpacing:-1.5}}>{scene.title}</h1>;
