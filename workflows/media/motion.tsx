...
function Workflow({scene,frame,color,muted,accent,light}:{scene:PreparedScene;frame:number;color:string;muted:string;accent:string;light:boolean}) {
  ...
  return <>
    <div style={{position:"absolute",left:58,top:90}}><Fit width={1164} height={83} id={`${scene.id}-workflow-heading`}><Words text={scene.title} frame={frame} size={scene.title.length>44?54:64} emphasis={scene.emphasis} color={color} accent={accent}/></Fit></div>
...
function Artifact({scene,frame,color,muted,accent}:{scene:PreparedScene;frame:number;color:string;muted:string;accent:string}) {
  ...
  return <>
    <div style={{position:"absolute",left:58,top:64}}><Fit width={1164} height={47} id={`${scene.id}-artifact-heading`}><Words text={scene.title} frame={frame} size={40} emphasis={scene.emphasis} color={color} accent={accent}/></Fit></div>
    {!details?<>
...
export function MotionScene({scene,index,total,restrained=false}:{scene:PreparedScene;index:number;total:number;restrained?:boolean}) {
  ...
  const huge=scene.title.length<15, titleSize=huge?174:scene.title.length<34?91:scene.title.length<56?72:60;
  ...
  return <AbsoluteFill style={{background:bg,color,fontFamily:FONT,overflow:"hidden"}}>
    ...
    {scene.math?<MathLayout {...{scene,color,muted}} accent={tone==="ink"?"#91a3ff":accent}/>:layout==="workflow"?<Workflow {...{scene,frame,color,muted,accent,light}}/>:layout==="task-field"?<Tasks {...{scene,frame,color,muted,accent,light,restrained}}/>:layout==="artifact-stage"&&scene.imageAsset?<Artifact {...{scene,frame,color,muted,accent}}/>:layout==="editorial"?<>
      <div style={{position:"absolute",left:58,top:118}}><Fit width={470} height={355} id={`${scene.id}-editorial-heading`}><Words text={scene.title} frame={frame} width={470} size={scene.title.length>40?58:72} emphasis={scene.emphasis} color={color} accent={accent}/></Fit></div>
      ...
      <div style={{position:"absolute",left:58,top:112}}><Fit width={1164} height={230} id={`${scene.id}-closing-title`}><Words text={scene.title} frame={frame} size={huge?180:80} emphasis={scene.emphasis} color={color} accent={accent}/></Fit></div>
