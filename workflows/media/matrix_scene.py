...
class MatrixScene(Scene):
    def construct(self):
        ...
        plane = NumberPlane(x_range=(-4, 4, 1), y_range=(-3, 3, 1), width=8, height=6,
                            faded_line_ratio=0, axis_config={"stroke_color": WHITE, "stroke_width": 2},
                            background_line_style={"stroke_color": BLUE, "stroke_width": 1, "stroke_opacity": .6})
        basis_x = Vector([1, 0, 0], color=TEAL, stroke_width=7)
        basis_y = Vector([0, 1, 0], color=YELLOW, stroke_width=7)
        square = Polygon(ORIGIN, [1, 0, 0], [1, 1, 0], [0, 1, 0], color=TEAL, fill_opacity=.18, stroke_width=2)
