/**
 * A language model plans a task once, from the request alone, before the first look.
 *
 * recipes.ts covers the everyday shapes with no model. For everything else this asks ONE text question, and asks
 * it for everything Jev cannot do itself:
 *
 ...
 *
 * A request for several things in different apps comes back as several tasks. The steps ride in `intent.steps`,
 * which is what `move` in screen.ts follows. Contacts go to the model by name only: it writes {email:Full Name}
 * and code fills in the address.
 */

import { type Intent, type JsonSchema, type Llm, toIntent } from "./intent.ts";
import type { Contact, Here } from "./recipes.ts";
...
export type PlanContext = {
  ...
  /** The page the worker's browser is on. */
  here?: Here | null;
...
export function toPlannedTask(raw: unknown, ctx: PlanContext): PlannedTask {
  ...
  const intent = toIntent({ goal: r.goal, url: ctx.app ? null : typeof r.url === "string" ? address(r.url, true) : r.url, done_when: r.done_when, avoid: r.avoid,
    inputs: inputs.map((i) => ({ name: i?.name, value: typeof i?.value === "string" ? address(i.value) : i?.value })) });
  if (!intent.url && !ctx.app && !ctx.here) throw new Error("it names no site, and the hand has no page open to carry on from");
...
export async function planTasks(llm: Llm, said: string, ctx: PlanContext): Promise<PlannedTask[]> {
  if (!said.trim()) throw new Error("nothing was said");
  const raw = (await llm({ system: planSystem(ctx), user: said.trim(), schema: PLAN_SCHEMA })) as { can_do?: unknown; tasks?: unknown };
