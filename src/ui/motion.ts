 ...
 * With reduced motion asked for, every duration here is nothing, and what depends on an animation ending still runs.
 */

const reduce = matchMedia("(prefers-reduced-motion: reduce)");

export const EASE_OUT = "cubic-bezier(0.23, 1, 0.32, 1)"; // what answers the user, and what comes in
export const EASE_EXIT = "cubic-bezier(0.3, 0, 0.8, 0.15)"; // what leaves for good
...
/** A card is swept off to the right, the way it came, and its place closes after it. Resolves once it is gone from the page. */
export async function sweep(card: HTMLElement): Promise<void> {
  card.inert = true;
  const height = card.offsetHeight;
  const off = card.animate([{ translate: "0 0", clipPath: "inset(-8px -8px -8px -8px)" }, { translate: "64px 0", clipPath: "inset(-8px -8px -8px 100%)" }], { duration: ms(200), easing: EASE_EXIT, fill: "forwards" });
  const shut = card.animate([{ height: `${height}px`, marginTop: "0px" }, { height: "0px", marginTop: `-${GAP}px` }], { duration: ms(240), delay: ms(120), easing: EASE_OUT, fill: "forwards" });
...
export function glide(from: Map<HTMLElement, number>): void {
  for (const [element, top] of from) {
    ...
    element.animate([{ transform: `translateY(${by}px)` }, { transform: "none" }], { id: "glide", duration: ms(320), easing: SPRING, composite: "add" });
  }
}

/** A word that has just changed, set down: it comes up a few pixels into place. Inside an opaque header, so it may fade. */
export function settle(element: HTMLElement): void {
  element.animate([{ opacity: 0, translate: "0 5px" }, { opacity: 1, translate: "0 0" }], { duration: ms(160), easing: EASE_OUT });
}
